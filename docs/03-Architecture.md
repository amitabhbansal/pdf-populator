...
## The Two Systems (Setup vs Runtime)

The solution splits into two independent systems joined by a single file (the mapping). **System 1** is a one-time human setup per template; **System 2** is the automated engine that runs for every record.

```mermaid
flowchart TB
    subgraph S1["SYSTEM 1 - Calibration - one-time, human"]
      P1["Scanned PDF"] --> P2["pdf_to_images.py"]
      P2 --> P3["Page images"]
      P3 --> P4["coordinate_calibrator.py - human clicks fields"]
    end
    P4 --> M["config/mappings/template.json<br/>positions in points + style"]
    subgraph S2["SYSTEM 2 - Rendering - automated"]
      O1["Original PDF"] --> R["pdf_renderer.py - validate + overlay"]
      D["Customer data JSON"] --> R
      R --> OUT["Filled PDF"]
    end
    M --> R
```

## Rendering Flow (System 2, in detail)
...
| Template configuration | `config/mappings/<template>.json` | Generated by the calibrator: each field's position (points) + style + type. Field names come from the data file's keys — there is no separate field list. |
